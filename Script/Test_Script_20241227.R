...
glu.net <- igraph_import("c:/Users/91879/OneDrive/Code/R/Projecct/2024.01.11.MSIP/Data/Glucose_to_glutatmate.network.xlsx")

cp.db <- MSdb:::get_KEGG_compound_df()
vdata(glu.net)$smiles <- vdata(glu.net)$id

a <- cp.db[match(vdata(glu.net)$id,cp.db$KEGG_id),]%>%
  as.data.frame()

a.pubchem <- webchem::pc_prop(cp.db$PubChem)
