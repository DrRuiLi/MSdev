#' @title edit_ms_condition
#' @description open an excel window to edit ms_conditon,
#' MSC_id to indicate which record to edit,
#'remove record if delete MSC_id in excel
...
edit_ms_condition <- function(MSC_id = "MSC0001") {
  ...
  ms_condition_to_edit <-
    ms_condition[which(ms_condition$MSC_id == MSC_id), ]
  ...
  ms_condition_table <-
    ms_condition_to_edit[-which(colnames(ms_condition_to_edit) %in% c("column", "phaseA", "phaseB", "gradient"))]

  ms_condition_table <-
    data.frame(ms_condition_table, row.names = "values") %>%
    t %>% as.data.frame() %>% rownames_to_column("params")

  ### edit in excel
  temp.xlsx <-
    paste0(tempdir(), "/temp_", paste0(sample(letters, 5), collapse = ""), ".xlsx")
  openxlsx::write.xlsx(
    list(
  ...
  ### load new data
  {
    ...
    ms_column <-
      openxlsx::read.xlsx(ms_workbook, sheet = "Column")
    ms_phase_A <-
      openxlsx::read.xlsx(ms_workbook, sheet = "phase A")
    ms_phase_B <-
      openxlsx::read.xlsx(ms_workbook, sheet = "phase B")
    ms_gradient <-
