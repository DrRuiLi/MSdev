...
#' @examples
setGeneric(
  name = "plot_xcms_peaks_distribution",
  def = function(xcms.xcms, plot.title = "Features distribution") {

    xcms.features <- featureDefinitions(xcms.xcms)%>%
      as.data.frame()%>%
      mutate(mz = mzmed,rt = rtmed)
    xcms.features$intb <- apply(featureValues(xcms.xcms,value = "intb"),1,mean,na.rm = T)
