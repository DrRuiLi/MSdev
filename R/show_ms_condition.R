...
return()
  ...
  ggplot(gradient)+
    ...
    labs(title = to_show$name,
         subtitle = paste0("Column : ",column$values[1]," , ",
                           column$values[4]," x ",column$values[5]," mm , ",
                           column$values[3], " μm\n",
                           "Phase A : ",paste0(str_c(phaseA$Concentration , phaseA$Compound,sep = " "),collapse = " + "),
                           "\nPhase B : ",paste0(str_c(phaseB$Concentration , phaseB$Compound,sep = " "),collapse = " + ")
         ))+
    ylim(c(0,100))+
    theme_bw()
