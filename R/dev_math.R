...
#' \enumerate{
#'   \item \code{\link{match_mz_foverlaps}} finds m/z candidate pairs within
#'     \code{mz.ppm} using \code{data.table::foverlaps()} on ppm-expanded intervals
#'     (denominator \code{mz1}).
#'   \item In \code{data.table}, absolute RT differences are computed and rows
#'     with \code{rt.error < rt.tol} (or \code{NA} RT error) are kept.
#' }
#' \code{mz.error} is the relative m/z error \code{abs(mz1 - mz2) / mz1}
#' (i.e. \code{abs(mz.ppm) / 1e6} from \code{match_mz_foverlaps}), not ppm units.
#' This function returns all surviving pairs; callers decide how to pick one
#' match per ion (e.g. closest RT then m/z in \code{get_Spectra_ms2_feature_id}).
...
#' @return A \code{data.table} with columns: \code{ion1} (index in first list),
#'   \code{ion2} (index in second list), \code{mz.error} (relative m/z error),
#'   and \code{rt.error} (absolute RT difference).
#' @seealso \code{\link{match_mz_foverlaps}}, \code{\link{match_mz}},
#'   \code{get_Spectra_ms2_feature_id}
#' @export
#'
match_mz_rt <- function(mz1, rt1 = rep(NA, length(mz1)),
  ...
  if (length(rt1) != length(mz1)) {
    stop("'rt1' must have the same length as 'mz1'", call. = FALSE)
  }
  if (length(rt2) != length(mz2)) {
    stop("'rt2' must have the same length as 'mz2'", call. = FALSE)
  }

  hit <- match_mz_foverlaps(
    mz1 = mz1,
    mz2 = mz2,
    ppm.base = mz1,
    ppm = mz.ppm
  ...
  ## match_mz_foverlaps returns mz.ppm; keep mz.error as relative fraction
  ## (abs(delta_mz)/mz1) for backward compatibility with callers.
  hit[, mz.error := abs(mz.ppm) / 1e6]
  hit[, rt.error := abs(rt2[ion2] - rt1[ion1])]
  hit <- hit[is.na(rt.error) | rt.error < rt.tol]
  hit[, .(ion1, ion2, mz.error, rt.error)]
}
