...
For many features or full-run traces, extract once with `get_xcms_feature_chromatogram()`, then plot with `plot_XChromatograms()` (section 6). `plot_xcms_feature_chromatogram()` extracts with that same engine and is meant for a handful of features.
...
`plot_xcms_xic()` is a ggplot2 stand-in for xcms `plot(..., type = "XIC")`. The upper-panel trace is extracted with `get_xcms_chromatogram()` (`aggregationFun = "sum"`):
